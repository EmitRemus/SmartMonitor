from meter import get_all_meter_ids, add_history_entries
from apartment_building import update_apartment_buildings
from pump_station import update_pipe_stations
import asyncio
...
async def get_data_from_mqtt(recieved_data: tuple):
    ...
    for meter_id, meter in meters.items():
        meter_insert = {
            "meter_id": meter_id,
            "entries": [
                {"date": meter["date"], "value": meter["value"]}
            ]
        }
