...
interface fetchAllPumpStationsReturns {
  data: TableDataPresentationData;
  onEndReached: () => void;
  isDataFinished: boolean;
}

export const useFetchAllPumpStations = (): fetchAllPumpStationsReturns => {
  const page = usePaginatedTableQuery(
    `${environment.API_BASE_URL}/pump-station/all`,
    ['pump-station', 'all'],
    20,
  );
...
function _parseData(
  ...
  for (const row of data.data) {
    ...
    const pumpStationId = row[0] as string | null;
